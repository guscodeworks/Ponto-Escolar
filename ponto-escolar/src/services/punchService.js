...
async function loginFuncionario(body, { ipOrigem } = {}) {
  ...
  const senhaCorreta = funcionario ? await bcrypt.compare(login.senha, String(funcionario.senha || '')) : false;
...
async function registerPunch({ funcionarioId, latitude, longitude }, { ipOrigem, userAgent } = {}) {
  ...
  try {
    const punch = await pointModel.withTransaction(async (tx) => {
      const funcionario = await employeeModel.findForPunchRegisterByIdForUpdate(tx, funcionarioId);
      ...
      const existingRow = await pointModel.findByEmployeeAndDateForUpdate(tx, funcionario.id, date);
